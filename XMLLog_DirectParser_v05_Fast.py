#!/usr/bin/env python3
"""
XMLLog_DirectParser_v05_Fast.py

Phase 2A multiprocessing parser for Sakura XML MasterLog extraction.
Version 05 - High Performance Build:
- Uses C-accelerated lxml.etree (with fallback to xml.etree.ElementTree)
- 16 MB buffered file reading for high-throughput SMB network streaming
- Multiprocessing file-level parallelism
"""
...
WANTED_FIELDS = {
    ...
    "Eng_MapNo", "Operator", "COMMENTS", "OperatorComment", "Eng_SerialNo", "Eng_Hours",
    ...
    "Spec2_NOxCycle", "Spec2_NOCycle", "Spec2_NO2Cycle",
    ...
    "Spec_CH4CycleCVS", "Spec2_CH4Cycle", "Spec_HCCycle", "Spec_COCycle",
    "Spec_PartCycleCVS", "Ma_Partf_1_Befor", "Ma_Partf_1_After",
    "FUEL_CONSUMPTION_GAL", "Mid_NO2_NOx_Ratio",
    "No_Samples", "Frq_Sampling", "Tm_LS", "Tm_MeasureStart",
}

FRONT_COLUMNS = [
    "Year", "Cell", "SourceFile", "SourcePath", "XMLModified",
    "TestID", "SubtestItem", "SubtestNum", "SubtestIndex", "SubtestID", "M_ID", "SubtestType",
    "SubTest Num", "RUN TYPE", "START TIME", "STOP TIME",
]
...
def is_timeseries_value(value: str, semicolon_threshold: int) -> bool:
    return bool(value) and value.count(";") > semicolon_threshold


def fast_par_scalar_local(par: Any, include_all_parameters: bool, semicolon_threshold: int, counters: dict[str, int]) -> tuple[str, str, bool]:
    ...
    for child in list(par):
        ...
        if child_tag == "NAME":
            ...
            if not name:
                continue
            raw = clean_text(child.text)
            if raw and not is_timeseries_value(raw, semicolon_threshold):
                value = raw
                break
            if raw:
...
def add_simple_derived(row: dict[str, Any]) -> None:
    ...
    if not row.get("Mid_NO2_NOx_Ratio"):
        row["Mid_NO2_NOx_Ratio"] = ratio(row.get("Spec5_NO2Cycle"), row.get("Spec5_NOxCycle"))
...
def parse_xml_file_worker(args: tuple[Any, ...]) -> tuple[list[dict[str, Any]], dict[str, Any], Optional[dict[str, str]]]:
    ...
    try:
        ...
        # Open file with 16 MB buffer to dramatically improve SMB network speed
        with xml_file.open("rb", buffering=16 * 1024 * 1024) as raw_fh:
            ...
            for event, elem in context:
                ...
                if tag == "ITEM" and in_content:
                    ...
                    for k, v in local.items():
                        set_first_nonblank(row, k, v)
                    row.update({"SubtestItem": item, "SubtestNum": subtest_num, "SubtestIndex": subtest_index, "SubtestID": subtest_id, "M_ID": m_id, "SubtestType": subtest_type, "SubTest Num": subtest_num})
                ...
                elif tag in {"CONTENT", "TESTDEF", "SUBTESTDEF", "MDEF", "M"}:
                    elem.clear()
                if stack:
                    stack.pop()
...
def write_workbook(rows: list[dict[str, Any]], errors: list[dict[str, Any]], file_stats: list[dict[str, Any]], out_xlsx: Path, total_seconds: float, workers: int) -> None:
    if not rows:
    ...
    summary = pd.DataFrame([
        ...
        {"Metric": "Rows/GB", "Value": round(rows_per_gb, 3) if rows_per_gb != "" else ""},
        {"Metric": "Seconds/GB", "Value": round(seconds_per_gb, 3) if seconds_per_gb != "" else ""},
    ...
    print(f"Unique SourceFiles            : {unique_files}")
    ...
    print(f"Rows/GB                       : {round(rows_per_gb, 3) if rows_per_gb != '' else ''}")
    print(f"Seconds/GB                    : {round(seconds_per_gb, 3) if seconds_per_gb != '' else ''}")
...
def run_sequential(files: list[Path], args, subtest_filters: list[str]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for i, xml_file in enumerate(files, start=1):
        ...
        task_args = (str(xml_file), args.include_all_parameters, args.semicolon_threshold, remaining_rows, not args.no_early_stop, args.skip_calibration, subtest_filters)
...
def run_parallel(files: list[Path], args, subtest_filters: list[str]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as ex:
        ...
        for xml_file in files:
            task_args = (str(xml_file), args.include_all_parameters, args.semicolon_threshold, None, not args.no_early_stop, args.skip_calibration, subtest_filters)
...
def main(argv: Optional[list[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="Phase 2A multiprocessing XML parser with benchmark metrics.")
    ...
    ap.add_argument("--out", default="Master_XML_Log_Direct_v05_Fast.xlsx", help="Output .xlsx file")
    ...
    print("\nDirect Sakura XML MasterLog Builder v05 Fast Build")
    print("=" * 72)
    ...
    write_workbook(rows, errors, file_stats, Path(args.out), total_seconds, args.workers)
